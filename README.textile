...
0) Do nothing, Cream automatically binds a session per invocation. 
...
h2. <a name="sch">Full Text Search</a>
...
h2. <a name="obs">Repository Observers</a>

To observe repository events create a class that implements javax.jcr.EventListener and annotate it with @OnJcrEvent. Cream will automatically register it for the current session.

bc.. @OnJcrEvent(eventTypes = Event.NODE_ADDED | Event.NODE_REMOVED | Event.PROPERTY_CHANGED, absPath = "/", isDeep = true)
